...
This workflow is based on the article:
...
Due to time and space limitations, we downloaded only a subset of the data, for
a real analysis please use all available data. The data used in the examples are
available in the package `r Biocpkg("TCGAWorkflowData")`.

## Installation

To be able to execute all the steps of this workflow please install the required
...
At the beginning of each section, the packages required to execute the code will
be loaded. However, the following packages are required for all sections.

- TCGAWorkflowData: This package contains the data necessary to execute each of
the analysis steps. This is a subset of the downloaded data to make the example
faster. For a real analysis, please use all the available data.
...
All the  package landing pages used in this workflow can be found through the
[biocViews interface](http://www.bioconductor.org/packages/release/BiocViews.html#___Software).

**Keywords:** Epigenomics, Genomics, Cancer, non-coding, TCGA, ENCODE, Roadmap,
Bioinformatics.
...
signatures are still far from being well characterized. Moreover,
...
a specific type of tissue or cell. When analyzed together, they provide
an invaluable opportunity for research laboratories to better understand
the developmental progression of normal cells to a cancer state at the
molecular level and importantly, correlate these phenotypes with tissues
of origin.
...
The previously stored data in CGHub, TCGA Data Portal and Broad Institute’s GDAC
Firehose, were provided as different levels or tiers that were defined in terms
of a specific combination of both processing level (raw, normalized, integrated)
and access level (controlled or open access). Level 1 indicated raw and
controlled data, level 2 indicated processed and controlled data, level 3
indicated Segmented or Interpreted Data and open access and level 4 indicated
region of interest and open access data. While the TCGA data portal provided
level 1 to 3 data, Firehose only provides level 3 and 4. An explanation of the
different levels can be found at [TCGA
Wiki](https://wiki.nci.nih.gov/display/TCGA/Data+level). However, the GDC data
portal no longer uses this level based classification model. Instead, a new data
model was created, its documentation can be found in [GDC
documentation](https://gdc.nci.nih.gov/developers/gdc-data-model/gdc-data-model-components).
In this new model, data can be open or controlled access. While the GDC open
access data does not require authentication or authorization to access it and
generally includes high-level genomic data that is not individually
identifiable, as well as most clinical and all biospecimen data elements, the
GDC controlled access data requires dbGaP authorization and eRA Commons
authentication and generally includes individually identifiable data such as
low-level genomic sequencing data, germline variants, SNP6 genotype data, and
certain clinical data elements. The process to obtain access to controlled data
is found in [GDC web
...
growth). All the data used in this workflow uses the Genome
Reference Consortium human genome (build 37 - hg19) as reference .
...
GDCdownload(query_exp.lgg)
exp.lgg <- GDCprepare(query = query_exp.lgg, save = TRUE, save.filename = "lggExp.rda")

query_exp.gbm <- GDCquery(
  project = "TCGA-GBM", 
  legacy = TRUE,
  data.category = "Gene expression",
  data.type = "Gene expression quantification",
...
GDCdownload(query_exp.gbm)
exp.gbm <- GDCprepare(query = query_exp.gbm, save = TRUE, save.filename = "gbmExp.rda")
...
If a
[SummarizedExperiment object](http://www.nature.com/nmeth/journal/v12/n2/fig_tab/nmeth.3252_F2.html)
was chosen, the data can be accessed with three different accessors: *assay* for
the data information, *rowRanges* to get the range of values in each row and
*colData* to get the sample information (patient, batch, sample type, etc)
[@huber2015orchestrating; @SummarizedExperiment]. An example is shown in the
listing below.
...
used as described in the listing below. This function has two
...
- dataset - Tumor to download. A complete list of possibilities can be view with
...
example is found in the listing below. The *getData* function
...
```{r results='hide', eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(TCGAbiolinks)
...
In order to visualize multiple genomic alteration events, we recommend using
maftools plot which is provided by Bioconductor package
[maftools](http://bioconductor.org/packages/maftools/)
[@mayakonda2016maftools]. The listing below shows how to download mutation data
using *GDCquery\_maf* (line 4) and prepare it to use with maftools.
...
We can draw oncoplot with the top 20 most mutated genes and add metadata
information such as molecular subtypes information.

```{r echo=TRUE, message=FALSE, warning=FALSE,fig.height=10,fig.width=15,eval=TRUE}
oncoplot(
  maf = maf,
  top = 20,
  legendFontSize = 8,
...
To pre-process the data, we first searched for possible outliers using the
*TCGAanalyze\_Preprocessing* function, which performs an Array Array Intensity
correlation (AAIC). This yields a square, symmetric matrix of pearson
correlation coefficients bewteen all samples in each cancer type (LGG or GBM).
This matrix identified 0 samples with low correlation (cor.cut = 0.6) which
could be identified as outliers.
...
in a confusion matrix as presented in the Table below.
...
following article [@ChIP-seqbioc]. The first step is shown in the listing
...
```{R tagHeatmap, eval=TRUE, include=TRUE}
...
```{R plotAvgProf, eval=TRUE, include=TRUE}
